...
public class ProductFacade extends AbstractFacade<Product> implements ProductFacadeLocal {
    ...
    public List<Product> getRelatedProducts(Product product, int limit) {
        TypedQuery<Product> q = em.createQuery("SELECT p FROM Product p "
                + "WHERE p.price = :price "
                + "OR p.brand.id = :brand "
                + "OR p.category.id = :cate", Product.class);
        return q.setParameter("brand", product.getBrand())
                .setParameter("cate", product.getCategory())
    ...
    public List<Product> getProductsByBrand(int brandId) {
        TypedQuery<Product> q = em.createQuery("SELECT p FROM Product p WHERE p.category.id = :brandId", Product.class);
